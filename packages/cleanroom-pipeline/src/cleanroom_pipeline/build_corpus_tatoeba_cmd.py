...
from cleanroom_pipeline.corpus_utils import make_cutlet, surface_reading_row


# Drop glue / punctuation as standalone candidates (full sentences still emitted separately).
...
def run_build_corpus_from_tatoeba(
    ...
    *,
    ...
    """Stream Tatoeba `jpn_sentences.tsv` (id TAB lang TAB text), emit morphemes + optional full line.

    *max_surfaces* counts unique *surface* strings written (dedupe by surface, first wins).
    """
    ...
    with tatoeba_sentences_tsv.open(encoding="utf-8") as fin:
        for line in fin:
            ...
            if include_full_sentences and len(seen) < max_surfaces:
                # Deck gates cap surface length at 64; strip trailing 。等 so looks_japanese passes.
